...
<div class="container">
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td><?= $etat['nom_etat']?></td>
            <td><a href="index.php?action=modifieretat&id=<?= $etat['id']?>" class="btn btn-warning">Modifier un etat</a>
                <a onclick="return confirm('&Ecirc;tes-vous sûr de vouloir supprimer ce ticket ?')"
                   style="color:red"
                   href="index.php?action=supprimerEtat&id=<?= $etat['id'] ?>">Supprimer un
                    ticket</a>
        </tr>
        <?php endforeach;?>
        </tbody>
